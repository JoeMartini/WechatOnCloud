...
> 🍴 **This fork adds OIDC unified authentication integration** — replace the native local login with Keycloak (or any OIDC provider) for centralized identity management. See [OIDC Integration](#oidc-integration) below.
...
This fork adds **OIDC unified authentication** to the WOC panel, replacing the native local login with centralized identity management via Keycloak (or any OIDC provider).

### What was changed

- **Auth Module** (`panel/server/src/auth/`):
  - Implemented OIDC authorization code flow with PKCE
  - Added `AuthProvider` abstraction supporting both `local` and `oidc` modes
  - File-based configuration to avoid env variable redaction in container runtimes (`/app/configs/woc/`)
  - ESM-compatible imports (`import { readFileSync }` instead of `require('fs')`)

- **Frontend** (`panel/web/src/hooks/useAuthConfig.ts`):
  - Dynamic auth mode detection from `/api/auth/mode`
  - Conditional rendering: shows SSO login button in OIDC mode, username/password in local mode

- **Session Management**:
  - JWT token validation from Keycloak
  - Role mapping from Keycloak `groups` claim to panel permissions

### How to use

1. Build the panel image from this fork
2. Mount OIDC configuration files:
   - `/app/configs/woc/auth_mode` — containing `oidc`
   - `/app/configs/woc/oidc_issuer_uri`
   - `/app/configs/woc/oidc_client_id`
   - `/app/configs/woc/oidc_secret`
3. Configure your IdP (Keycloak) with the WOC callback URL
4. Start the panel container — users will be redirected to the IdP login page

### Branches in this fork

| Branch | Purpose |
|--------|---------|
| `main` | Community OIDC integration (tracks upstream) |
...
- [x] **OIDC 统一身份认证集成** — 使用 Keycloak 替代原生登录
- [ ] 掉登录时 web 端二维码重扫入口
